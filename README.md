...
`MKFramework` 是一个专为 `Cocos Creator 3.x` 设计的综合游戏开发框架。它提供了一系列集成系统，用于处理常见的游戏开发任务，例如`资源管理`、`UI 管理`、`音频播放`、`本地化`和`网络`。
...
顶部菜单：`扩展/MKFramework/安装框架/稳定版本`

等待安装成功后重启编辑器在新建脚本内输入 `mk` 即可看到框架的接口提示
